...
namespace Voxeland5
{
	...
	public class Chunk : MonoBehaviour, IChunk, ISerializationCallbackReceiver
	{
		...
		[SerializeField] private CoordDir[] indexToCoord; //no need to make it public, should be used only by Chunk
		...
		public void OnCreate (object voxelandBox) 
		{
			...
			if (voxeland.hideColliderWire) meshCollider.hideFlags = HideFlags.HideInHierarchy;
		...
		public void OnMove (Coord oldCoord, Coord newCoord) 
		{
			...
			if (voxeland.copyComponents)
			{
				...
				for (int i=0; i<components.Length; i++)
				{
					...
					if (gameObject.GetComponent(components[i].GetType()) == null) Extensions.CopyComponent(components[i], go);
				}
			}
		...
		#region Mesh
			public void CalculateMesh ()	
			{
				...
				//types
				if (meshWorker.stop) return;
				hiWrapper.tangents = ChunkMesh.CalculateChannels(faces, verts, numCh:voxeland.landTypes.array.Length, simpleEncode:voxeland.encodeChannelsToColors);
				loWrapper.tangents = hiWrapper.tangents.Truncated(loWrapper.verts.Length);
				if (voxeland.encodeChannelsToColors)
				{
					hiWrapper.colors = hiWrapper.tangents.ToColors();
					loWrapper.colors = loWrapper.tangents.ToColors();
					hiWrapper.tangents = null; loWrapper.tangents = null;
				}
