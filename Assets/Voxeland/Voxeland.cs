...
namespace Voxeland5
{
	...
	#endif
	{
		public static readonly int version = 500;   ///<For serialization purpose', to load older versions as a special cases.
		public static Extensions.VersionState versionState = Extensions.VersionState.release;   ///<For serialization and debug purposes.
		...
		public bool encodeChannelsToColors = false;
		...
		public void OnEnable ()
		{
			...
			if (!UnityEditor.EditorApplication.isPlaying)
			{
				...
				if (prefab)
				{
					...
					if (material == null) { material = GetDefaultLandMaterial(); landTypes.ApplyToMaterial(material); }
					if (farMaterial == null) { farMaterial = GetDefaultFarMaterial(); landTypes.ApplyToMaterial(farMaterial); }
		...
		#region Events and actions
			...
			{ 
				...
				landTypes.ApplyToMaterial(farMaterial);
			...
			public void Generate (bool force=false) //called on generator change
			{
				...
				//data.areas.Clear(); //data.areas.RemoveNonPinned();
		...
		#region MapMagic
			...
			public MapMagic.Generator previewGenerator {get;set;}
			public MapMagic.Generator.Output previewOutput {get;set;}
			...
			public void Highlight (CoordDir center, Brush brush, bool isEditing=false) //isEditing for displaying blob brush
			{
				...
				else if (chunk != null)
				{
					if (brush.form == Brush.Form.single) 
					{
						...
						{
							highlight.AddVoxelandFace(blockCoord, chunk, opacity:0.33f);
						}
					}
					...
					else if (brush.form == Brush.Form.blob )
					{
						...
						else 
						{
							...
							for (int n=0; n<neigCoords.Length; n++) 
							{
								Coord neigChunkCoord = Coord.PickCell(neigCoords[n].x, neigCoords[n].z, chunkSize);
								highlight.AddVoxelandFace(neigCoords[n], chunks[neigChunkCoord]);
							}
						}
					}
