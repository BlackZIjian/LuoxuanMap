	...
	static public class Extensions
	{
		...
		public static Vector3[] GetCamPoses (bool genAroundMainCam=true, string genAroundTag=null, Vector3[] camPoses=null)
		{
			if (IsEditor()) 
			{
                ...
                try
                {
                    centers = GameObject.FindGameObjectsWithTag("SceneCenter");
                    if(centers == null || centers.Length == 0)
                    {
                        centers = new GameObject[1] { UnityEditor.SceneView.lastActiveSceneView.camera.gameObject };
                    }
                }
                catch
                {
                ...
                if (camPoses==null || camPoses.Length!= centers.Length) camPoses = new Vector3[centers.Length];
                for(int i=0;i<centers.Length;i++)
                {
                    camPoses[i] = centers[i].transform.position;
                }
